...
var credits;
//Instruction
var instructionViewImg = new Image();
var instructions;
...
function Main()
{
	...
	instructionBImg.src = 'Img/instruction.png';
	instructionBImg.name = 'instructionB';
	//instructionBImg.onload = loadGfx;
	
	creditsViewImg.src = 'Img/credits.png';
	creditsViewImg.name = 'credits';
	creditsViewImg.onload = loadGfx;

	instructionViewImg.src = 'Img/instructionview.png';
	instructionViewImg.name = 'instructions';
	//instructionViewImg.onload = loadGfx;
...
function loadGfx(e)
{
	...
	if(e.target.name = 'instructionB'){instructionB = new Bitmap(instructionBImg);}
	if(e.target.name = 'instructions'){instructions = new Bitmap(instructionViewImg);}
...
function addTitleView()
{
	...
	instructionB.x =241 - 49;
	instructionB.y =240;
	
	TitleView.addChild(main, startB, creditsB, instructionB);
	...
	instructionB.onPress = showInstruction;
}
...
function showInstruction()
{
	...
	stage.update();
	Tween.get(instructions).to({x:0}, 300);
	instructions.onPress = hideInstruction;
}
...
{
	Tween.get(instructions).to({x:480}, 300).call(rmvInstruction);
}
...
function update()
{
	...
	if(ball.x + 40 > cpu.x && ball.x + 30 < cpu.x + 22 && ball.y >= cpu.y && ball.y < cpu.y + 75)
	{
		xSpeed *= -1;
		SoundJS.play('hit');
	}
